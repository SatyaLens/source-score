// Code generated by counterfeiter. DO NOT EDIT.
package sourcefakes

import (
	"context"
	"source-score/pkg/api"
	"source-score/pkg/domain/source"
	"sync"
)

type FakeSourceRepository struct {
	DeleteSourceByUriDigestStub        func(context.Context, *api.Source) error
	deleteSourceByUriDigestMutex       sync.RWMutex
	deleteSourceByUriDigestArgsForCall []struct {
		arg1 context.Context
		arg2 *api.Source
	}
	deleteSourceByUriDigestReturns struct {
		result1 error
	}
	deleteSourceByUriDigestReturnsOnCall map[int]struct {
		result1 error
	}
	GetSourceByUriDigestStub        func(context.Context, string) (*api.Source, error)
	getSourceByUriDigestMutex       sync.RWMutex
	getSourceByUriDigestArgsForCall []struct {
		arg1 context.Context
		arg2 string
	}
	getSourceByUriDigestReturns struct {
		result1 *api.Source
		result2 error
	}
	getSourceByUriDigestReturnsOnCall map[int]struct {
		result1 *api.Source
		result2 error
	}
	PostSourceStub        func(context.Context, *api.SourceInput) error
	postSourceMutex       sync.RWMutex
	postSourceArgsForCall []struct {
		arg1 context.Context
		arg2 *api.SourceInput
	}
	postSourceReturns struct {
		result1 error
	}
	postSourceReturnsOnCall map[int]struct {
		result1 error
	}
	PutSourceByUriDigestStub        func(context.Context, *api.SourceInput, string) error
	putSourceByUriDigestMutex       sync.RWMutex
	putSourceByUriDigestArgsForCall []struct {
		arg1 context.Context
		arg2 *api.SourceInput
		arg3 string
	}
	putSourceByUriDigestReturns struct {
		result1 error
	}
	putSourceByUriDigestReturnsOnCall map[int]struct {
		result1 error
	}
	invocations      map[string][][]interface{}
	invocationsMutex sync.RWMutex
}

func (fake *FakeSourceRepository) DeleteSourceByUriDigest(arg1 context.Context, arg2 *api.Source) error {
	fake.deleteSourceByUriDigestMutex.Lock()
	ret, specificReturn := fake.deleteSourceByUriDigestReturnsOnCall[len(fake.deleteSourceByUriDigestArgsForCall)]
	fake.deleteSourceByUriDigestArgsForCall = append(fake.deleteSourceByUriDigestArgsForCall, struct {
		arg1 context.Context
		arg2 *api.Source
	}{arg1, arg2})
	stub := fake.DeleteSourceByUriDigestStub
	fakeReturns := fake.deleteSourceByUriDigestReturns
	fake.recordInvocation("DeleteSourceByUriDigest", []interface{}{arg1, arg2})
	fake.deleteSourceByUriDigestMutex.Unlock()
	if stub != nil {
		return stub(arg1, arg2)
	}
	if specificReturn {
		return ret.result1
	}
	return fakeReturns.result1
}

func (fake *FakeSourceRepository) DeleteSourceByUriDigestCallCount() int {
	fake.deleteSourceByUriDigestMutex.RLock()
	defer fake.deleteSourceByUriDigestMutex.RUnlock()
	return len(fake.deleteSourceByUriDigestArgsForCall)
}

func (fake *FakeSourceRepository) DeleteSourceByUriDigestCalls(stub func(context.Context, *api.Source) error) {
	fake.deleteSourceByUriDigestMutex.Lock()
	defer fake.deleteSourceByUriDigestMutex.Unlock()
	fake.DeleteSourceByUriDigestStub = stub
}

func (fake *FakeSourceRepository) DeleteSourceByUriDigestArgsForCall(i int) (context.Context, *api.Source) {
	fake.deleteSourceByUriDigestMutex.RLock()
	defer fake.deleteSourceByUriDigestMutex.RUnlock()
	argsForCall := fake.deleteSourceByUriDigestArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2
}

func (fake *FakeSourceRepository) DeleteSourceByUriDigestReturns(result1 error) {
	fake.deleteSourceByUriDigestMutex.Lock()
	defer fake.deleteSourceByUriDigestMutex.Unlock()
	fake.DeleteSourceByUriDigestStub = nil
	fake.deleteSourceByUriDigestReturns = struct {
		result1 error
	}{result1}
}

func (fake *FakeSourceRepository) DeleteSourceByUriDigestReturnsOnCall(i int, result1 error) {
	fake.deleteSourceByUriDigestMutex.Lock()
	defer fake.deleteSourceByUriDigestMutex.Unlock()
	fake.DeleteSourceByUriDigestStub = nil
	if fake.deleteSourceByUriDigestReturnsOnCall == nil {
		fake.deleteSourceByUriDigestReturnsOnCall = make(map[int]struct {
			result1 error
		})
	}
	fake.deleteSourceByUriDigestReturnsOnCall[i] = struct {
		result1 error
	}{result1}
}

func (fake *FakeSourceRepository) GetSourceByUriDigest(arg1 context.Context, arg2 string) (*api.Source, error) {
	fake.getSourceByUriDigestMutex.Lock()
	ret, specificReturn := fake.getSourceByUriDigestReturnsOnCall[len(fake.getSourceByUriDigestArgsForCall)]
	fake.getSourceByUriDigestArgsForCall = append(fake.getSourceByUriDigestArgsForCall, struct {
		arg1 context.Context
		arg2 string
	}{arg1, arg2})
	stub := fake.GetSourceByUriDigestStub
	fakeReturns := fake.getSourceByUriDigestReturns
	fake.recordInvocation("GetSourceByUriDigest", []interface{}{arg1, arg2})
	fake.getSourceByUriDigestMutex.Unlock()
	if stub != nil {
		return stub(arg1, arg2)
	}
	if specificReturn {
		return ret.result1, ret.result2
	}
	return fakeReturns.result1, fakeReturns.result2
}

func (fake *FakeSourceRepository) GetSourceByUriDigestCallCount() int {
	fake.getSourceByUriDigestMutex.RLock()
	defer fake.getSourceByUriDigestMutex.RUnlock()
	return len(fake.getSourceByUriDigestArgsForCall)
}

func (fake *FakeSourceRepository) GetSourceByUriDigestCalls(stub func(context.Context, string) (*api.Source, error)) {
	fake.getSourceByUriDigestMutex.Lock()
	defer fake.getSourceByUriDigestMutex.Unlock()
	fake.GetSourceByUriDigestStub = stub
}

func (fake *FakeSourceRepository) GetSourceByUriDigestArgsForCall(i int) (context.Context, string) {
	fake.getSourceByUriDigestMutex.RLock()
	defer fake.getSourceByUriDigestMutex.RUnlock()
	argsForCall := fake.getSourceByUriDigestArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2
}

func (fake *FakeSourceRepository) GetSourceByUriDigestReturns(result1 *api.Source, result2 error) {
	fake.getSourceByUriDigestMutex.Lock()
	defer fake.getSourceByUriDigestMutex.Unlock()
	fake.GetSourceByUriDigestStub = nil
	fake.getSourceByUriDigestReturns = struct {
		result1 *api.Source
		result2 error
	}{result1, result2}
}

func (fake *FakeSourceRepository) GetSourceByUriDigestReturnsOnCall(i int, result1 *api.Source, result2 error) {
	fake.getSourceByUriDigestMutex.Lock()
	defer fake.getSourceByUriDigestMutex.Unlock()
	fake.GetSourceByUriDigestStub = nil
	if fake.getSourceByUriDigestReturnsOnCall == nil {
		fake.getSourceByUriDigestReturnsOnCall = make(map[int]struct {
			result1 *api.Source
			result2 error
		})
	}
	fake.getSourceByUriDigestReturnsOnCall[i] = struct {
		result1 *api.Source
		result2 error
	}{result1, result2}
}

func (fake *FakeSourceRepository) PostSource(arg1 context.Context, arg2 *api.SourceInput) error {
	fake.postSourceMutex.Lock()
	ret, specificReturn := fake.postSourceReturnsOnCall[len(fake.postSourceArgsForCall)]
	fake.postSourceArgsForCall = append(fake.postSourceArgsForCall, struct {
		arg1 context.Context
		arg2 *api.SourceInput
	}{arg1, arg2})
	stub := fake.PostSourceStub
	fakeReturns := fake.postSourceReturns
	fake.recordInvocation("PostSource", []interface{}{arg1, arg2})
	fake.postSourceMutex.Unlock()
	if stub != nil {
		return stub(arg1, arg2)
	}
	if specificReturn {
		return ret.result1
	}
	return fakeReturns.result1
}

func (fake *FakeSourceRepository) PostSourceCallCount() int {
	fake.postSourceMutex.RLock()
	defer fake.postSourceMutex.RUnlock()
	return len(fake.postSourceArgsForCall)
}

func (fake *FakeSourceRepository) PostSourceCalls(stub func(context.Context, *api.SourceInput) error) {
	fake.postSourceMutex.Lock()
	defer fake.postSourceMutex.Unlock()
	fake.PostSourceStub = stub
}

func (fake *FakeSourceRepository) PostSourceArgsForCall(i int) (context.Context, *api.SourceInput) {
	fake.postSourceMutex.RLock()
	defer fake.postSourceMutex.RUnlock()
	argsForCall := fake.postSourceArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2
}

func (fake *FakeSourceRepository) PostSourceReturns(result1 error) {
	fake.postSourceMutex.Lock()
	defer fake.postSourceMutex.Unlock()
	fake.PostSourceStub = nil
	fake.postSourceReturns = struct {
		result1 error
	}{result1}
}

func (fake *FakeSourceRepository) PostSourceReturnsOnCall(i int, result1 error) {
	fake.postSourceMutex.Lock()
	defer fake.postSourceMutex.Unlock()
	fake.PostSourceStub = nil
	if fake.postSourceReturnsOnCall == nil {
		fake.postSourceReturnsOnCall = make(map[int]struct {
			result1 error
		})
	}
	fake.postSourceReturnsOnCall[i] = struct {
		result1 error
	}{result1}
}

func (fake *FakeSourceRepository) PutSourceByUriDigest(arg1 context.Context, arg2 *api.SourceInput, arg3 string) error {
	fake.putSourceByUriDigestMutex.Lock()
	ret, specificReturn := fake.putSourceByUriDigestReturnsOnCall[len(fake.putSourceByUriDigestArgsForCall)]
	fake.putSourceByUriDigestArgsForCall = append(fake.putSourceByUriDigestArgsForCall, struct {
		arg1 context.Context
		arg2 *api.SourceInput
		arg3 string
	}{arg1, arg2, arg3})
	stub := fake.PutSourceByUriDigestStub
	fakeReturns := fake.putSourceByUriDigestReturns
	fake.recordInvocation("PutSourceByUriDigest", []interface{}{arg1, arg2, arg3})
	fake.putSourceByUriDigestMutex.Unlock()
	if stub != nil {
		return stub(arg1, arg2, arg3)
	}
	if specificReturn {
		return ret.result1
	}
	return fakeReturns.result1
}

func (fake *FakeSourceRepository) PutSourceByUriDigestCallCount() int {
	fake.putSourceByUriDigestMutex.RLock()
	defer fake.putSourceByUriDigestMutex.RUnlock()
	return len(fake.putSourceByUriDigestArgsForCall)
}

func (fake *FakeSourceRepository) PutSourceByUriDigestCalls(stub func(context.Context, *api.SourceInput, string) error) {
	fake.putSourceByUriDigestMutex.Lock()
	defer fake.putSourceByUriDigestMutex.Unlock()
	fake.PutSourceByUriDigestStub = stub
}

func (fake *FakeSourceRepository) PutSourceByUriDigestArgsForCall(i int) (context.Context, *api.SourceInput, string) {
	fake.putSourceByUriDigestMutex.RLock()
	defer fake.putSourceByUriDigestMutex.RUnlock()
	argsForCall := fake.putSourceByUriDigestArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2, argsForCall.arg3
}

func (fake *FakeSourceRepository) PutSourceByUriDigestReturns(result1 error) {
	fake.putSourceByUriDigestMutex.Lock()
	defer fake.putSourceByUriDigestMutex.Unlock()
	fake.PutSourceByUriDigestStub = nil
	fake.putSourceByUriDigestReturns = struct {
		result1 error
	}{result1}
}

func (fake *FakeSourceRepository) PutSourceByUriDigestReturnsOnCall(i int, result1 error) {
	fake.putSourceByUriDigestMutex.Lock()
	defer fake.putSourceByUriDigestMutex.Unlock()
	fake.PutSourceByUriDigestStub = nil
	if fake.putSourceByUriDigestReturnsOnCall == nil {
		fake.putSourceByUriDigestReturnsOnCall = make(map[int]struct {
			result1 error
		})
	}
	fake.putSourceByUriDigestReturnsOnCall[i] = struct {
		result1 error
	}{result1}
}

func (fake *FakeSourceRepository) Invocations() map[string][][]interface{} {
	fake.invocationsMutex.RLock()
	defer fake.invocationsMutex.RUnlock()
	copiedInvocations := map[string][][]interface{}{}
	for key, value := range fake.invocations {
		copiedInvocations[key] = value
	}
	return copiedInvocations
}

func (fake *FakeSourceRepository) recordInvocation(key string, args []interface{}) {
	fake.invocationsMutex.Lock()
	defer fake.invocationsMutex.Unlock()
	if fake.invocations == nil {
		fake.invocations = map[string][][]interface{}{}
	}
	if fake.invocations[key] == nil {
		fake.invocations[key] = [][]interface{}{}
	}
	fake.invocations[key] = append(fake.invocations[key], args)
}

var _ source.SourceRepository = new(FakeSourceRepository)
